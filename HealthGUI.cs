...
namespace Assets.Scripts
{
    class HealthGUI : MonoBehaviour
    {
        ...
        void OnGUI()
        {
            if (!Data.PlayerDead)
            {
                GUI.DrawTexture(new Rect(Screen.width * .05f, Screen.height * (.051f + (.3f / (float)player.MAX_HEALTH) * (player.MAX_HEALTH - player.Health)),
            ...
            if (Data.Enemy!=null)
            {
                GUI.DrawTexture(new Rect(Screen.width * .9f, Screen.height * (.051f + (.3f / (float)Data.Enemy.maxHealth) * (Data.Enemy.maxHealth - Data.Enemy.Health)),
