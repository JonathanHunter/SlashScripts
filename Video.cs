...
namespace Assets.Scripts
{
    class Video : MonoBehaviour
    {
        ...
        void Update()
        {
            if (CustomInput.CancelUp || (cursor == (int)State.Cancel && CustomInput.AcceptUp))
            ...
            if (cursor == (int)State.Resolution && (CustomInput.AcceptUp || CustomInput.RightUp))
            {
                resIndex++;
            ...
            if (cursor == (int)State.Quality && (CustomInput.AcceptUp || CustomInput.RightUp))
            {
                quality++;
        ...
        {
            //left, top, width, height
            if (GUI.Button(new Rect(Screen.width * (10f / 19f), Screen.height * (4.5f / 12f), Screen.width * (3f / 19f), Screen.height * (1f / 12f)), Screen.resolutions[resIndex].width + "x" + Screen.resolutions[resIndex].height, LabelStyle))
            {
                resIndex++;
                if (resIndex >= Screen.resolutions.Length)
                    resIndex = 0;
            ...
            if (GUI.Button(new Rect(Screen.width * (10f / 19f), Screen.height * (7.5f / 12f), Screen.width * (3f / 19f), Screen.height * (1f / 12f)), QualitySettings.names[quality].ToString(), LabelStyle))
            {
                quality++;
                if (quality >= QualitySettings.names.Length)
                    quality = 0;
