...
{
	class LevelTwoEnemy : MonoBehaviour
	{
		...
		private LevelTwoEnemyStateMachine machine;
		...
		void Start()
		{
			...
			health = MAX_HEALTH;
			machine = new LevelTwoEnemyStateMachine();
		...
		void Update()
		{
			if (!Data.Paused)
			{
				...
				switch (state)
				{
				case (int)LevelTwoEnemyStateMachine.State.Idle: Idle(transform); break;
				case (int)LevelTwoEnemyStateMachine.State.Hit: Hit(transform); break;
				case (int)LevelTwoEnemyStateMachine.State.Walk: Walk(transform); break;
				case (int)LevelTwoEnemyStateMachine.State.Turn: Turn(transform); break;
				}
				if (beingHit)
					beingHit = false;
				health -= damage;
				if (damage > 0)
					damage = 0;
				if (health <= 0 && state != (int)LevelTwoEnemyStateMachine.State.Hit)
				{
